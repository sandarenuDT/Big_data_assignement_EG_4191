...
└── src/main/java/com/assignment/kafka/
    ├── OrderProducer.java    # Produces orders to Kafka
    ├── OrderConsumer.java    # Consumes and processes orders
    ├── DlqConsumer.java      # Handles failed messages
    └── AvroUtils.java        # Avro serialization utilities
```
...
### 2️⃣ Start Schema Registry on WSL
...
```
DLQ Received → key=1007 | orderId=1007 | product=Item3 | price=46.12
DLQ Received → key=1004 | orderId=1004 | product=Item2 | price=28.90
DLQ Received → key=10013 | orderId=10013 | product=Item5 | price=73.41
```
...
```json
{
  ...
  "fields": [
    ...
    },
    {
      "name": "product",
      "type": "string",
      "doc": "The product name or code associated with the order. One of Item1–Item5 in this assignment."
    },
    {
      "name": "price",
      "type": "float",
      "doc": "The total price of the order. A floating-point value between 20–100 as generated by the producer."
    }
  ]
}
```
